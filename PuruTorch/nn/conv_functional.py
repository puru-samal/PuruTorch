...
class Pad1D(Function):
    ...
    def forward(self, A:Tensor, padding:int=0) -> Tensor:
        ...
        return Tensor(padded_A, requires_grad, self if requires_grad else None)
...
class MaxPool1D_stride1(Function):
    ...
    def forward(self, A:Tensor, kernel:int) -> Tensor:
        ...
        maxindex = np.empty((N, C_out, W_out), dtype=tuple)
        Z = np.zeros((N, C_out, W_out))
        ...
        for batch in range(N):
            for ch in range(C_out):
                for w in range(W_out):
                    ...
                    maxindex[batch, ch, w] = np.unravel_index(np.argmax(scan), scan.shape)
                    maxindex[batch, ch, w] = tuple(np.add((w), maxindex[batch, ch, w]))
        
        requires_grad = A.requires_grad
        if requires_grad:
            self.ctx.save_for_backward(A)
            self.ctx.maxindex = maxindex
...
class MaxPool2D_stride1(Function):
    ...
    def forward(self, A:Tensor, kernel:int) -> Tensor:
        ...
        maxindex = np.empty((N, C_out, H_out, W_out), dtype=tuple)
        Z = np.zeros((N, C_out, H_out, W_out))
            ...
            for ch in range(C_out):
                for h in range(H_out):
                    for w in range(W_out):
                        ...
                        maxindex[batch, ch, h, w] = np.unravel_index(np.argmax(scan), scan.shape)
                        maxindex[batch, ch, h, w] = tuple(np.add((h, w), maxindex[batch, ch, h, w]))
